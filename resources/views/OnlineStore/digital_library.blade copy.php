...
@include('OnlineStore.includes.poster')
                     ...
                     <div class="fashion_section_2">
                        <div class="row">
                           <?php 
                                ...
                                $getproducts=App\Http\Controllers\Store\StoreController::getLibraryProducts($section->id,3,$i);
    ...
    box-shadow: 5px 5px 5px black;"></div>
                                 <div class="btn_main">
                                    <div class="buy_bt"><a class="btn" onclick="checkLogin({{$product->id}},'digital_library','{{$product->name}}',{{$product->sale_price}},{{$product->dp ?? 0}},{{$product->bv ?? 0}},'{{$product->supplier_name}}','{{$product->supplier_number}}','{{$product->account_type}}')">Buy Now</a></div>
                                    <div class="seemore_bt"><a href="library-details/{{$product->id}}">See More</a></div>
                                 </div>
                              </div>
                           </div>
